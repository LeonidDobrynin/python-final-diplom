...
class RegisterAccount(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if {"username", "email", "password", "company", "position", }.issubset(request.data):
            errors = {}
...
class СategoriesListView(ListAPIView):
    """
...
class BasketView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.user.type == "shop":
            return JsonResponse({"Status": False, "Error": "Не для магазинов"}, status=403)
        ...
        if items_sting:
            try:
                ...
                for order_item in items_dict:
                    try:
                        product = ProductInfo.objects.get(id=order_item["product_info"]).product_id
                        shop = ProductInfo.objects.get(id=order_item["product_info"]).shop_id
                    except ProductInfo.DoesNotExist as error:
                        return JsonResponse({"Status": False, "Errors": str(error)})
                    order_item.update({"order": basket.id, "product": product, "shop": shop})
    ...
    def put(self, request, *args, **kwargs):
        ...
        if items_sting:
            try:
                ...
                objects_updated = 0
                for order_item in items_dict:
                    order_item_qs = OrderItem.objects.filter(order_id=basket.id, id=order_item["id"])
